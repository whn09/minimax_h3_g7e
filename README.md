...
**唯一有收益的旋钮是 Cache-DiT**：1.35–1.92× 可调，**有损但在等成本口径下优于减步数**。它不是
...
**RDT 0.16 与 0.20 在 768p/30 上输出逐格相同**（0.20/0.24 在 480p/30 上同样相同）——gate 命中同一批步，
别把它们当两档报。步数越多可复用的 residual 越多，所以 30 步的倍数（2.0–2.4×）明显高于 20 步（1.4–1.9×）。
...
| 场景 | base | Cache-DiT | 省 |
|---|---|---|---|
| 768p/30 | $0.016291 | **$0.007397**（RDT 0.16） | −54.6% |
| 480p/30 | $0.004332 | **$0.001991**（RDT 0.20） | −54.0% |
| 768p/20 | $0.010951 | $0.006755（RDT 0.16） | −38.3% |
| 480p/20 | $0.003001 | $0.001829（RDT 0.20） | −39.1% |

30 步 + Cache-DiT（768p $0.007397）**比 20 步 base（$0.010951）还便宜 32.4%**。
...
- **DiT cache / 跳步**：外挂的三套（TeaCache 等）层次上接不上 H3。sglang **自带**的 Cache-DiT 在
  c0b6474 上真挂上了、1.35–1.92× 可调，等成本口径下**优于减步数**（+0.021 SSIM）——**可选加速档**，
